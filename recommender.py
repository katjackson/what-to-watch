...
movie_ids_over_5_ratings = get_significantly_rated_movie_ids(5, ratings_by_id)


def get_movie_ids_by_avg_rating(ratings_by_id):
    # Returns a sorted list of tuples (movie_id, avg rating)
    movie_id_avg_rating_list = [(movie_id,
                                Rating.get_average_rating(
                                    movie_id, ratings_by_id))
                                for movie_id in ratings_by_id]
    return sorted(movie_id_avg_rating_list,
                  key=lambda movie_and_rating: movie_and_rating[1], reverse=True)


def get_highest_rated_movies(movie_id_list, ratings_by_id, movie_object_list):
    """ Takes a list of movie_ids (like significantly rated ones or ones
    that a user has not rated), a dicts of ratings, and movie objects for
    getting titles. Returns a list of movie titles """

    movie_ratings = []
    movie_ids_by_avg_rating = get_movie_ids_by_avg_rating(ratings_by_id)
    movie_names_in_order = [Movie.get_name(tuple_thing[0], movie_object_list)
                            for tuple_thing in movie_ids_by_avg_rating
                            if tuple_thing[0] in movie_id_list]
...
def make_userless_list(user_id, all_rating_data, all_movie_data):
    ...
    # user_ratings is a list of (movie id, rating) tuples
    user_ratings = Rating.get_user_ratings(user_id, all_rating_data)
    list_of_movie_ids = [rating_tuple[0] for rating_tuple in user_ratings]
    list_of_unrated_ids = [movie.movie_id for movie in all_movie_data
                          if movie.movie_id not in list_of_movie_ids]
    return list_of_unrated_ids
...
def get_best_movies_for_user(user_id, rating_data, movie_data, ratings_by_id):
    ...
    userless_ids = [movie_id for movie_id in userless_ids if movie_id
                    in (get_significantly_rated_movie_ids(5, ratings_by_id))]
...
def get_matched_ratings_for_2_users(user_id_1, user_id_2, ratings_by_user):
    ...
    movie_ids_in_both = [x[0] for x in user_1_ratings
                        for y in user_2_ratings if x[0] == y[0]]
    user_1_matched_ratings = [x[1] for x in user_1_ratings
                             if x[0] in movie_ids_in_both]
    user_2_matched_ratings = [x[1] for x in user_2_ratings
                             if x[0] in movie_ids_in_both]
...
def find_euc_dist_of_taste(user_id_1, user_id_2, ratings_by_user):
    list_1, list_2 = get_matched_ratings_for_2_users(user_id_1, user_id_2,
                                                    ratings_by_user)
...
def find_like_users(user_1, ratings_by_user, all_user_data):
    ...
    for user in all_user_data:
        if user.user_id != user_1:
            ed = find_euc_dist_of_taste(user_1, user.user_id, ratings_by_user)
            list_of_euc_dist.append((user.user_id, ed))
...
def get_ratings_from_like_users(like_users, ratings_by_user, ratings_by_id):
    like_user_rating_tuples = [rating_tuple for user in like_users
                              for rating_tuple in ratings_by_user[user]]
...
def get_recommendations_for_user(
        user_id, ratings_by_user=ratings_by_user,
        ratings_by_id=ratings_by_id, rating_data=all_rating_data,
        movie_data=all_movie_data):
    users_like_user = find_like_users(user_id, ratings_by_user, all_user_data)
    like_user_ratings_by_id = get_ratings_from_like_users(users_like_user,
                                                          ratings_by_user,
                                                          ratings_by_id)
    return get_best_movies_for_user(
            user_id, rating_data, movie_data, like_user_ratings_by_id
            )
